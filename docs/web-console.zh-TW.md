...
- Conversation view：保存多輪 sessions，並預覽哪些歷史訊息會進入 context。
- Prompt Lab view：預覽 rendered prompt、比較 prompt templates，並切換 inference modes。
- External view：比較 local model 與 OpenAI-compatible 或 Ollama provider。
- Deploy view：檢查 runtime limits，並估算 inference/training resource shape。
...
- Chat SFT view：freeze GPT-2，用 multi-turn chat transcripts 訓練 LoRA，然後到 Conversation 測試 checkpoint。
...
Web UI 頂部會顯示同一份 runtime 資訊。CPU 可以用來跑 tiny from-scratch 學習階段；GPT-2 instruction SFT、LoRA、Chat SFT 若要在合理時間完成，應該使用 CUDA。CPU 只建議當成很短的 smoke test。設定步驟請看 [PyTorch GPU Runtime 設定](gpu-runtime.zh-TW.md)。
...
4. 打開 Conversation，送 `My name is Rojar. Please remember it.`，再 preview `What is my name?`。
5. 比較 session 保存的 transcript 和模型 `context_length` warning。
6. 打開 Prompt Lab，用同一個 message 預覽 `raw`、`chat`、`instruction`、`custom`，再比較 `greedy`、`focused`、`creative`。
7. 設定真正 external provider，再打開 External 和 local model 比較。
8. 檢查 provider request preview，確認瀏覽器端不會拿到 provider credentials。
9. 打開 Deploy，先估算 `random-tiny-byte`，再增加 context 和 concurrency，觀察哪些成本變大。
10. 打開 From Scratch，跑 `every-effort`，比較 before/after。
11. 打開 Raw Text；UI 應該選到 `the-verdict`，並建議 `random-tiny-byte` 和 `trained-verdict-byte`。
12. 跑 The Verdict job，觀察較大資料上的 raw text continuation。
13. 打開 GPT-2，載入 `GPT-2 small`。
14. 回到 Chat，問一個 instruction-style request，例如 `Explain what a model checkpoint is in one sentence.`
15. 打開 Instruction；UI 應該選到 `instruction-following`，並顯示三步驟閉環：instruction data、GPT-2 base、instruction SFT。
16. 如果 instruction data 還不存在，按 `Download dataset`。面板接著應該顯示一筆 dataset example 和格式化後的 Chapter 7 model input。
17. 載入 `GPT-2 small`，執行 instruction SFT，然後比較 `Before (raw GPT-2)` 和 `After (instruction SFT)`。
18. 打開 LoRA；UI 應該選到 `instruction-lora`，並顯示 LoRA adapter training。
19. 執行 LoRA，並比較 trainable parameter percentage 和 full instruction SFT 的差異。
20. 打開 Chat SFT；UI 應該選到 `chat-sft-lora`，並顯示 multi-turn chat transcript training。
21. 用 CUDA 執行 Chat SFT，然後載入 `gpt2-chat-lora`。
22. 打開 Conversation，選 `gpt2-chat-lora`，使用 `Chat transcript`，測試 `My name is Rojar` 接著問 `What is my name?`。
23. 打開 Dataset Builder；檢視種子資料，新增一筆 `train` 範例與一筆 `eval` 範例。
24. 使用 `instruction-builder` 執行 custom SFT，並比較 `Before (GPT-2 base)` 與 `After (custom SFT)`。
25. 打開 Experiments，比較 raw pretrained GPT-2、full SFT、LoRA、Chat SFT 和 custom SFT。
26. 先讀 comparison summary，再讀 generated samples。
27. 打開 Checkpoints，檢視 model version lineage 並載入指定版本。
28. 回到 Chat，送出 streaming request，並在到達 `max_new_tokens` 前取消。
...
最小 chat fine-tuning 請看 [最小 GPU Chat Model](minimal-chat-model.zh-TW.md)。
...
外部 provider 請看 [外部模型整合](external-model-integration.zh-TW.md)。
部署資源限制請看 [部署與資源限制](deployment-resource-limits.zh-TW.md)。
多輪 sessions 請看 [多輪對話與會話記憶](conversation-memory.zh-TW.md)。
