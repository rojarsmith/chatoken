...
| 參數量 | 136,704 | 163,037,184 |

也就是在同一份程式碼裡，參數量大約多了 **1,190 倍**。權重從 `openai-community/gpt2` 下載，
再由 `_load_hf_gpt2_weights` 複製進模組樹中——這個函式把 Hugging Face 的參數名稱對應到本專案的
層名稱。讀懂它，就是「GPT-2」與「你自己寫的模型」是同一個架構最清楚的證明。

**為什麼不是 124,000,000。** Hugging Face 會把 GPT-2 的輸出頭與 token embedding *綁定*
（同一份權重矩陣兼任兩者），而「124M」就是那個綁定後的數字：124,439,808。本專案保留了獨立的
`out_head`，因此多存了一份 `emb_dim × vocab_size` 矩陣：`768 × 50,257 = 38,597,376`。
兩者相加，正好是 API 回報的 163,037,184。權重內容完全相同；那個 head 是 `wte` 的複本，
不是新的知識。
...
開啟 `http://127.0.0.1:3000`，在階梯上選 **Stage 08 · Pretrained GPT-2**。
