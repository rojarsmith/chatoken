...
> **控制台狀態。** 階梯已上線於 `http://127.0.0.1:3000`。Stage 01–03 已有互動面板；
> 其餘階段會出現在階梯上，並連到自己的文件以及目前仍持有那些操作的 legacy 頁籤。
> 原本的 16 頁籤控制台保留在 `/legacy`，直到所有階段都遷移完成。
> 時程請見[架構重整規劃](restructure-plan.zh-TW.md)。
